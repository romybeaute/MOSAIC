...
DATASETS = {
    "API Translation (Batched)": '/Users/rbeaute/Projects/MOSAIC/DATA/multilingual/MPE_dataset_translated_batched.csv',
    "Local Translation (Llama)": '/Users/rbeaute/Projects/MOSAIC/DATA/multilingual/MPE_dataset_translated_llama.csv'
}
HISTORY_FILE = "run_history.json"

CACHE_DIR = "/Users/rbeaute/Projects/MOSAIC/DATA/multilingual/english/mpe/cache"
...
A:"""
    ...
    llm_labels_cleaned = []
    for label in llm_labels_raw:
        if ':' in label: label = label.split(':')[-1]
        cleaned_label = label.strip().strip('"').strip('.').strip()
        llm_labels_cleaned.append(cleaned_label)
...
def generate_and_save_embeddings(csv_path, docs_file, embeddings_file, selected_embedding_model, split_sentences):
        ...
        if split_sentences:
            with st.spinner('Splitting into sentences...'):
                try: nltk.data.find('tokenizers/punkt')
                    ...
                    st.info("Downloading 'punkt' tokenizer..."); nltk.download('punkt')
    ...
    st.info(f"Generating embeddings with '{selected_embedding_model}' for {len(docs)} documents...")
    with st.spinner("This can take a while..."):
        embedding_model_obj = SentenceTransformer(selected_embedding_model)
        embeddings = embedding_model_obj.encode(docs, show_progress_bar=True)
    with st.spinner(f"Saving embeddings..."):
        np.save(embeddings_file, embeddings)
    st.success("Embedding generation complete!"); st.balloons(); st.rerun()

# --- 4. MAIN APP INTERFACE ---
st.title("Advanced MPE Topic Modeling Dashboard")
...
# --- NEW CODE BLOCK ---
st.sidebar.markdown(
    "[See model performance on the MTEB Leaderboard](https://huggingface.co/spaces/mteb/leaderboard)"
)

MODEL_SPECIFICATIONS = {
    "intfloat/multilingual-e5-large-instruct": {
        "description": "A powerful multilingual model, great for diverse languages. Requires more computational resources.",
        "size": "2.24 GB",
        "speed": "Medium",
    },
    "Qwen/Qwen3-Embedding-0.6B": {
        "description": "A very strong multilingual embedding model from Alibaba Cloud. Recommended for best results.",
        "size": "0.6B parameters",
        "speed": "Fast",
    },
    "BAAI/bge-small-en-v1.5": {
        "description": "A fast and efficient English-only model. Good for quick experiments on English text.",
        "size": "67 MB",
        "speed": "Very Fast",
    },
    "sentence-transformers/all-mpnet-base-v2": {
        "description": "A classic, well-balanced model for English. A solid baseline choice.",
        "size": "438 MB",
        "speed": "Fast",
    }
}
...
st.sidebar.header("Data Granularity")
...
def get_precomputed_filenames(csv_path, model_name, split_sentences):
    ...
    # --- MODIFICATION ---
    # Join the cache directory with the filenames
...
if not os.path.exists(EMBEDDINGS_FILE):
    ...
    if st.button("Prepare Data for this Configuration"):
        generate_and_save_embeddings(CSV_PATH, DOCS_FILE, EMBEDDINGS_FILE, selected_embedding_model, selected_granularity)
else:
    ...
    #     ngram_min = st.slider("Min N-gram", 1, 5, DEFAULT_PARAMS['ngram_min'])
    #     ngram_max = st.slider("Max N-gram", 1, 5, DEFAULT_PARAMS['ngram_max'])
    #     min_df = st.slider("Min Doc Frequency", 1, 50, DEFAULT_PARAMS['min_df'],step=5)
    #     stopwords = st.select_slider("Stopwords", ['english', None], value=DEFAULT_PARAMS['stopwords'])

    # with st.sidebar.expander("UMAP Parameters", expanded=True):
    #     umap_neighbors = st.slider("n_neighbors", 2, 50, DEFAULT_PARAMS['umap_neighbors'])
    #     umap_components = st.slider("n_components", 2, 50, DEFAULT_PARAMS['umap_components'])
    #     umap_min_dist = st.slider("min_dist", 0.0, 0.99, DEFAULT_PARAMS['min_dist'], step=0.01)
        
    # with st.sidebar.expander("HDBSCAN Parameters", expanded=True):
    #     hdbscan_min_cluster_size = st.slider("min_cluster_size", 5, 100, DEFAULT_PARAMS['hdbscan_min_cluster_size'],step=5)
    #     hdbscan_min_samples = st.slider("min_samples", 5, 100, DEFAULT_PARAMS['hdbscan_min_samples'],step=5)
    
    # with st.sidebar.expander("BERTopic Parameters", expanded=True):
    #     bt_nr_topics = st.text_input("Number of Topics (nr_topics)", value=DEFAULT_PARAMS['bt_nr_topics'])
    #     bt_top_n_words = st.slider("Top N Words", 5, 25, DEFAULT_PARAMS['bt_top_n_words'])

    with st.sidebar.expander("CountVectorizer Parameters", expanded=True):
        ngram_min = st.slider(
            "Min N-gram", 1, 5, DEFAULT_PARAMS['ngram_min'],
            help="The minimum size of word sequences to consider as a keyword. A value of 1 means single words (unigrams)."
        )
        ngram_max = st.slider(
            "Max N-gram", 1, 5, DEFAULT_PARAMS['ngram_max'],
            help="The maximum size of word sequences. A value of 3 means sequences of up to three words (trigrams) can be keywords."
        )
        min_df = st.slider(
            "Min Doc Frequency", 1, 50, DEFAULT_PARAMS['min_df'], step=5,
            help="The minimum number of documents a word must appear in to be considered a keyword. Helps remove very rare words."
        )
        stopwords = st.select_slider(
            "Stopwords", ['english', None], value=DEFAULT_PARAMS['stopwords'],
            help="Removes common words (like 'the', 'is', 'a'). Select 'english' for English text. Select 'None' to not remove any."
        )

    with st.sidebar.expander("UMAP Parameters", expanded=True):
        umap_neighbors = st.slider(
            "n_neighbors", 2, 50, DEFAULT_PARAMS['umap_neighbors'],
            help="Controls how UMAP balances local versus global structure. Lower values focus on local details, higher values on the bigger picture."
        )
        umap_components = st.slider(
            "n_components", 2, 50, DEFAULT_PARAMS['umap_components'],
            help="The number of dimensions to reduce the data to before clustering. The default is often sufficient."
        )
        umap_min_dist = st.slider(
            "min_dist", 0.0, 0.99, DEFAULT_PARAMS['min_dist'], step=0.01,
            help="Controls how tightly UMAP packs points together. Lower values create more dense clusters, higher values create more dispersed ones."
        )
        
    with st.sidebar.expander("HDBSCAN Parameters", expanded=True):
        hdbscan_min_cluster_size = st.slider(
            "min_cluster_size", 5, 100, DEFAULT_PARAMS['hdbscan_min_cluster_size'], step=5,
            help="The smallest number of documents required to form a distinct topic. Higher values lead to fewer, broader topics."
        )
        hdbscan_min_samples = st.slider(
            "min_samples", 5, 100, DEFAULT_PARAMS['hdbscan_min_samples'], step=5,
            help="How conservative the clustering is. Higher values make the model more likely to declare documents as outliers rather than assigning them to a topic."
        )

    with st.sidebar.expander("BERTopic Parameters", expanded=True):
        bt_nr_topics = st.text_input(
            "Number of Topics (nr_topics)", value=DEFAULT_PARAMS['bt_nr_topics'],
            help="Set to a specific number to merge topics until you have that many. Set to 'auto' to let HDBSCAN decide."
        )
        bt_top_n_words = st.slider(
            "Top N Words", 5, 25, DEFAULT_PARAMS['bt_top_n_words'],
            help="The number of keywords to generate for each topic's basic representation."
        )
    ...
    current_config = {
        ...
        "vectorizer_params": {'ngram_range': (ngram_min, ngram_max), 'min_df': min_df, 'stop_words': stopwords},
        "umap_params": {'n_neighbors': umap_neighbors, 'n_components': umap_components, 'min_dist': umap_min_dist},
        "hdbscan_params": {'min_cluster_size': hdbscan_min_cluster_size, 'min_samples': hdbscan_min_samples},
        "bt_params": {'nr_topics': bt_nr_topics, 'top_n_words': bt_top_n_words}
    }
    ...
    if run_button:
        with st.spinner('Performing topic modeling...'):
            ...
            history_entry = { 
                ...
                "llm_labels": [l for l in topic_model.get_topic_info().Name.values if "Unlabelled" not in l and "outlier" not in l]
            }
    ...
    with history_tab:
        ...
        if not st.session_state.history:
            st.info(f"No runs yet. History will be saved to `{HISTORY_FILE}`.")
        else:
            for i, entry in enumerate(st.session_state.history):
                ...
                with st.expander(title):
                    ...
                    with st.expander("Show full configuration for this run"):
                        st.json(config_data)
