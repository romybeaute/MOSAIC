...
def secure_local_translation(csv_path: str, output_path: str, model_config: dict, text_column: str = 'report', num_samples: int = None):
    """
    ...
    df_to_process['report_english'] = translations
    df_to_process.to_csv(output_path, index=False)
    
    print(f"\n--- Translation Complete ---")
    print(f"Translated data saved to {output_path}")
    print(df_to_process[[text_column, 'report_english']].head())
...
"{text_to_translate}"<|eot_id|><|start_header_id|>assistant<|end_header_id|>"""
    ...
    #file in data directory will be in DATA_DIR, called "f{database}_reports.csv"

    INPUT_CSV_PATH = os.path.join(DATA_DIR, f"{database}_reports.csv")
    output_filename = f'translations/{database}_translated_{MODEL_CHOICE}.csv'
    OUTPUT_CSV_PATH = os.path.join(DATA_DIR, output_filename)
