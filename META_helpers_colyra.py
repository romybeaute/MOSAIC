...
def hyperparams(len_dataset):
    ...
    return {'umap_params': {
        ...
        'hdbscan_params': {
            ...
            'min_cluster_size': [int(len_dataset*0.025),int(len_dataset*0.05)],
            'min_samples': [None]#[int(len_dataset*0.025),int(len_dataset*0.05)] #default to None
        }
    }
...
def hyperparams_reduced(len_dataset):
    ...
    '''
    return {'umap_params': {
            'n_components': [3,5,7], #default to 2 
...
def run_grid_search(data, vectorizer_model, embedding_model,HighSensory,HandWritten=False,reduced_GS=False,store_results=True):
    ...
    if store_results:
        name_file = f'RESULTS2/grid_search_results_{"HighSensory" if HighSensory else "DeepListening" if not HandWritten else "HandWritten"}_seed{random_state}.csv'
